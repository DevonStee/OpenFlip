...
- 使用自定义 `Canvas` + 3D 变换实现高拟真翻页动画 —— 每次翻页都模拟了纸牌重量、阴影与透视效果。
- 极简全屏时钟界面，强调交互触感，每个操作都带有触觉反馈。
- **模拟灯泡光效**：温暖的 Light Overlay 再现台灯照射钟表的质感，点击开关带有舒适的触觉反馈。
- **旋钮调时交互**：拖动旋钮以可变速度快速翻过数字，模拟机械调节转盘的惯性手感。
- **整点报时**：整点播放可配置的报时音效，可在设置中调节音量或开关。
- **定时关闭**：设置倒计时后自动退出应用 —— 非常适合床头使用，避免整夜耗电。
